...
impl BookShelf {
    ...
    fn find_book<S: ToString>(&self, item: S) -> Option<&InerBook> {
...
impl BookShelf{
    pub fn add_book<B: Book>(&mut self, book: B) -> bool{
...
impl BookShelf {
    ...
    fn add_book_to_author(&mut self, book: Id, author: Id) {
        if let Some(person) = self.authors.get_by_id_mut(author)
        //I don't think we will end up with multible book entryes so not going to mess with it for now
        {person.works.push(book);}
    }

    fn add_book_to_publisher(&mut self, book: Id, publisher: Id) -> bool{
        if let Some(publisher) = self.publishers.get_by_id_mut(publisher){
    ...
    fn add_book_to_series(&mut self, book: Id, series: Id) -> bool{
        if let Some(series) = self.series.get_by_id_mut(series){
    ...
    fn add_book_to_narator(&mut self, book: Id, narator: Id) -> bool {
        if let Some(narator) = self.narators.get_by_id_mut(narator){
